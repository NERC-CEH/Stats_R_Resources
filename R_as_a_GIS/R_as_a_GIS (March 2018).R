# The script has been tested in R x64 3.3.3
...
setwd("C:/Users/dcha/Downloads/Stats_R_Resources-master/R_as_a_GIS")
...
hist(UK$area, br=50, col="grey")
hist(log(UK$area), br=50, col="grey")

# Plot chloropleth map of UK, shaded by area
spplot(UK, "area")

# Dissolve the UK multi-part polygons to a single UK outline 
...
plot(rWater, col="blue", maxpixels=ncell(rWater))
...
plot(aggregate(rWater, fact=10, fun="sum", na.rm=TRUE), col=bpy.colors(100),
     zlim=c(0,100))
plot(UK_out_simple, add=TRUE)
...
# so the points are re-projected on the fly
...
summary(X)
apply(X, 2, sd) # standard deviation of each variable
...
plot(Variogram(G1, form=~x+y, resType="pearson", maxDist=5), smooth=TRUE)
...
sc = spline.correlog(x=coordinates(rndPts)[,1]/1000, 
...
rangeValues = seq(0.3, 5, 0.1) # values of the range parameter to test
rangeTest = sapply(rangeValues, function(r){
	message("range = ", r*100, " km") # print the fixed range parameter to screen
...
bestRange = rangeTest[1, which.max(rangeTest[2,])] # find the range giving max likelihood
abline(v=bestRange, col="red", lty=2)
